...
QImage Binary::renderDisplay(QSize viewportSize, const QJsonObject &parameters, QSharedPointer<PluginActionProgress> progress)
{
    Q_UNUSED(progress)
    m_lastParams = parameters;

    return DisplayHelper::drawTextRasterFull(viewportSize, headerOffset(parameters), this, m_handle, parameters, 1,
...
QImage Binary::renderOverlay(QSize viewportSize, const QJsonObject &parameters)
{
    if (!m_delegate->validate(m_lastParams)) {
        return QImage();
    }
